...
export default class AvatarPEPPlugin extends AbstractPlugin {
   ...
   public static setAvatar(connection: IConnection, hash: string, data: string, imgheight: string, imgwidth: string, mimetype: string, size: string) {

      let item = $build('metadata',{xmlns:'urn:xmpp:avatar:metadata'}).c('info',{bytes:size,id:hash,height:imgheight,width:imgwidth,type:mimetype}).tree();
